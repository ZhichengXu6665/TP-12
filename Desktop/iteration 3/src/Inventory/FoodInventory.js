...
import ImageButtons from './ImageButtons'; // 根据你的文件路径调整导入路径
...
function FoodInventory({ onExpiredItemsUpdate }) {
    ...
    const handleConsume = (index) => {
        ...
        const updatedInventory = [...inventory]; // 创建副本，避免直接修改状态
        const itemToMove = updatedInventory[index];
        const today = new Date().toISOString().split("T")[0];
        
        if (confirmation) {
            // 如果用户确认完全使用该物品，从 inventory 中移除该物品
            const usedItem = {
                ...itemToMove,
