...
const normalizeQuantity = (quantityValue, quantityUnit) => {
  const conversionFactor = UNIT_CONVERSIONS[quantityUnit] || 1; // Default conversion factor is 1
  const normalizedQuantity = quantityValue * conversionFactor;
  console.log(`Normalized Quantity: ${normalizedQuantity} from ${quantityValue} ${quantityUnit}`);
  return normalizedQuantity;
};

// New function to parse quantity string if necessary
const parseQuantity = (quantityString) => {
  if (!quantityString) return { quantityValue: 0, quantityUnit: '' };
  const regex = /([\d.]+)\s*(\w*)/;
  const match = quantityString.match(regex);
  if (match) {
    const quantityValue = parseFloat(match[1]);
    const quantityUnit = match[2];
    return { quantityValue, quantityUnit };
  }
  return { quantityValue: 0, quantityUnit: '' };
};

function AnalyticsCharts({ selectedOption, categoryData = [], expiredItems = [] }) {
  const [pieData, setPieData] = useState([]);
  const [costData, setCostData] = useState([]);
  const [carbonFootprintData, setCarbonFootprintData] = useState([]); // Added state for carbon footprint data
  ...
  useEffect(() => {
    ...
    const totalEmissionsData = categoryData.map((category, index) => {
      const itemsInCategory = expiredItems.filter((item) => {
        const itemCategory = item.category ? item.category.trim().toLowerCase() : '';
        const categoryName = category.name ? category.name.trim().toLowerCase() : '';
        return itemCategory === categoryName;
      ...
      const totalEmissions = itemsInCategory.reduce((sum, item) => {
        // Parse quantity and unit
        let { quantityValue, quantityUnit } = item;
        if (!quantityValue || !quantityUnit) {
          // If quantityValue or quantityUnit is missing, parse from quantity string
        ...
        const normalizedQuantity = normalizeQuantity(quantityValue, quantityUnit);
        const emissionFactor = EMISSION_FACTORS[category.name] || 1;
        const itemEmissions = normalizedQuantity * emissionFactor;
        return sum + itemEmissions;
      }, 0);
    ...
    setCarbonFootprintData(totalEmissionsData);
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                ...
                fill="#8884d8"
                label={({ name, value }) =>
                  `${name}: ${typeof value === 'number' ? value.toFixed(2) : value} kg CO₂e`
                } // Show labels with units
                labelLine
              ...
              <Tooltip
                formatter={(value) =>
                  typeof value === 'number' ? `${value.toFixed(2)} kg CO₂e` : value
                }
              />
              <Legend
                formatter={(value) =>
